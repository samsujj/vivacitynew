	...
	else
	{
		?>
			<div class="te te_delete_form">
				<form class="te te_delete_form" name="frm" method="post" action="<?php echo htmlspecialchars($this->url( "te_mode=delete&te_key=" . $this->te_key  )); ?>">
					<input class="confirm_button" type="submit" name="btnConfirm" value="Confirm Delete" />
					<input class="cancel_button" type="button" name="btnCancel" value="Cancel" onclick="document.location='<?php echo htmlspecialchars($this->url('te_mode=table')); ?>';" />
				</form>
			</div>
		<?php
	}
