 ...
 * share_links module
 */
class pixel_module extends module_base
{
	public $mod_system_name = 'pixel'; // Not static because parent needs to access this
	public $mod_name = 'Pixel Manager';
	public $mod_description = 'Pixel Manager';
	...
	public function mod_upgrade( $db_version )
	{
		...
		if ( $this->mod_is_older_version($db_version, '.1') )
		{
			...
			$AI->skin->create_dynamicpage(
				'pixel' //$pagename
				, array('body' => 'includes/modules/pixel/pixel.php') //$content
			...
			$AI->grant_multiple_perms( $perm_classes, $perm_groups, $perm_types, false );
			$AI->grant_page_perm( 'pixel', array('Website Developers','Administrators','Users','Anonymous','Distributor') );
		...
		if ( $this->mod_is_older_version($db_version, '2.4') ) {
			...
			$perm_types = array('ajax','ajax_cmd_inline_edit','ajax_cmd_inline_save','asearch','copy','delete','insert','multidelete','table','update','view');
			$AI->grant_multiple_perms( $perm_classes, $perm_groups, $perm_types, false );
			$AI->grant_page_perm( 'pixel', array('Distributors') );
		}
	...
	{
		echo '<p>This module creates a pixel manager database.</p>';
	}
