<?php
	...
	class C_te_share_asset extends C_tableedit_base
	{

		var $upload_dir = 'uploads/share_asset/';

		//(configure) parameters
		var $_dbTableName = 'share_asset';
		var $_keyFieldName = 'id';
		var $_numeric_key = true;
		var $unique_id = 'share_asset';
		...
		//(configure) Draw Code
		var $view_include_file = 'includes/modules/share_asset/includes/draw.share_asset.view.php';
		var $edit_include_file = 'includes/modules/share_asset/includes/draw.share_asset.edit.php';
		var $table_include_file = 'includes/modules/share_asset/includes/draw.share_asset.table.php';
		var $qsearch_include_file = 'includes/modules/share_asset/includes/draw.share_asset.qsearch.php';
		var $asearch_include_file = 'includes/modules/share_asset/includes/draw.share_asset.asearch.php';
		var $viewnav_include_file = 'includes/modules/share_asset/includes/draw.share_asset.viewnav.php';
		var $noresults_include_file = 'includes/modules/share_asset/includes/draw.share_asset.noresults.php';
		var $ajax_include_file = 'includes/modules/share_asset/includes/handler.share_asset.ajax.php';
		...
		function C_te_share_asset( $param_dbWhere = '' )
		{
			...
			$this->inline_edit_db_field['height'] = 'none';//( $this->perm->get('ajax_cmd_inline_edit') ? 'all' : 'none' );
			$this->inline_edit_db_field['width'] = 'none';//( $this->perm->get('ajax_cmd_inline_edit') ? 'all' : 'none' );
			...
			$this->te_permit['insert_share_asset'] = $this->perm->get('insert_share_asset');
...
$type = 1;
if($this->db['type'] == 'Banner Ads'){
$type = 1;
}
if($this->db['type'] == 'Display Ads'){
$type = 2;
}
if($this->db['type'] == 'Mobile Ads'){
$type = 3;
}
...
$type = 3;
		...
		function draw_input_field( $fieldname, $value, $mode, $element_id = '' )
		{
			...
			switch( $fieldname )
			{
				...
				case 'url': {
					$AI->skin->js('includes/modules/share_asset/share_asset.js');
		...
		function draw_image_list($id=0,$type=""){
			...
			if(count($pixel_arr)){
				...
				foreach($pixel_arr as $p) {
					...
					$img_url = (file_exists($p['url']))?'<img src="http://www.vivacitygo.com/image?imgurl='.$p['url'].'&w='.$p['width'].'&h='.$p['height'].'&ar=1&e=0&cr=0" style="border: solid 1px #ccc; display:block; margin:0 auto; max-width:100%" />':'';

					echo '<div style="width: 96%; padding:20px  2% 5px; border-bottom: solid 1px #ccc;">';
					echo $img_url;
					echo '<textarea id="txtarea'.$p['id'].'" style="width:90%; box-shadow: none; display: block;  min-height: 20px; border: solid 1px #ccc; resize: none; margin: 25px auto; max-width: 430px; height: 110px;"><a href="'.$share_link_url.'" target="_blank"><img src="http://'.$_SERVER['HTTP_HOST'].'/'.$p['url'].'" alt="'.$p['name'].'" border="0" /></a></textarea>';
					...
					echo '<button class="info clip1btn" style="height:30px; padding-top: 5px; display: block; margin: 0 auto;" data-clipboard-target="#txtarea'.$p['id'].'">Grab Code</button>';
					echo '</div>';
