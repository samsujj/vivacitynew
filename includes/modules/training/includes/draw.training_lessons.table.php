...
<!--
	function training_lessons_delete_selected()
	{
		var frm = document.getElementById('training_lessons_table_form');
	...
	function training_lessons_process_multiselect_checkbox( selectall_checkbox )
	{
		var frm = document.getElementById('training_lessons_table_form');
	...
	function training_lessons_update_sort_index(table, row)
	{
		// Fix Zebra Stripping
...
<div class="te_table training_lessons_table">
	<fieldset class="te">
		<legend class="te">
			<?php echo htmlspecialchars( $this->_tableTitle ); ?>
		</legend>

		<form id="training_lessons_table_form" class="te_table_form" name="training_lessons_table_form" method="post" action="<?php echo htmlspecialchars($this->url( '' )); ?>">
			<table class="te_main_table training_lessons_main_table" id="training_lessons_main_table">
				<thead>
					<tr class="te_fieldname_row">
						<?php
							...
							$this->draw_FieldName('category_id', 'Category');
							$this->draw_FieldName('name', 'Name');
							$this->draw_FieldName('prerequisite_id', 'Prerequisite Lesson');
				...
				<tbody>
					<?php
						...
						for($table_i=0; $table_i < $this->_pgSize && $table_row; $table_i++)
						{
							...
							echo "\n".'<!-- Category Id --> <td class="te_data_cell ' . ( $table_i % 2 == 1 ? 'te_even_cell' : 'te_odd_cell' ) . '" id="value_field_container_' . $this->db[ $this->_keyFieldName ] . '_category_id">'; $this->draw_value_field( 'category_id', $this->db['category_id'].'', $this->db[ $this->_keyFieldName ], 'table' ); echo '</td>';
							...
							echo "\n".'<!-- Prerequisite Id --> <td class="te_data_cell ' . ( $table_i % 2 == 1 ? 'te_even_cell' : 'te_odd_cell' ) . '" id="value_field_container_' . $this->db[ $this->_keyFieldName ] . '_prerequisite_id">'; $this->draw_value_field( 'prerequisite_id', $this->db['prerequisite_id'].'', $this->db[ $this->_keyFieldName ], 'table' ); echo '</td>';
							...
							echo "\n".'<!-- Sort Order --> <td class="te_data_cell ' . ( $table_i % 2 == 1 ? 'te_even_cell' : 'te_odd_cell' ) . '" id="value_field_container_' . $this->db[ $this->_keyFieldName ] . '_sort_order" style="text-align:right">'; $this->draw_value_field( 'sort_order', $this->db['sort_order'].'', $this->db[ $this->_keyFieldName ], 'table' ); echo '</td>';
